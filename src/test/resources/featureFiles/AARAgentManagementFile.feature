Feature: AAR Agent Management File Process Feature
  ...
  Scenario Outline: Validate AAR Agent Management File
   ...
   #   | "ANC"     | "ACV"   | "ebhlaunch" | "chrome" | "SMRITIDHUNGANA" | "Legendary@1" | "[EBHLaunch].[dbo].[usp_GetAARagentManagementDetailInfo]" | "[EBHLaunch].[dbo].[usp_GetAARagentManagementData]" |
      | "ANC"     | "ACV"     | "ebhstaging" | "MicrosoftEdge" | "SmritiTest" | "Legendary@1" | "[EBH].[dbo].[usp_GetAARagentManagementDetailInfo]" | "[EBH].[dbo].[usp_GetAARagentManagementData]" |



  #23
  @Regression @AARReassignment @FailureScreenShot1
  Scenario Outline:(SET-2195) Add Logic to Check Settling Table Upon AAR Reassignment
    Given Run Test for <environment> on Browser <browser> for AAR Agent Management and Enter the url
    And Login to the Agents Portal with username <username> and password <password> for AAR Agent Management
    And Navigate to the Corporate Page on Main Menu and to the Settlements page for AAR Agent Management
    And Navigate to the AAR Agent Management File page
  ...
  #  And Ensure the new Agent is included in the Settlement Summary <VendorCode>
    ...
    #  | "10008"    | "SMRITIDHUNGANA" | "SJAGGI"  | "33"      | "settle" | "ebhlaunch" | "[EBHLaunch].[dbo].[SETTLING_TABLE]" | "chrome" | "SMRITIDHUNGANA" | "Legendary@1" |
     # | "10008"    | "SMRITIDHUNGANA" | "SJAGGI"  | "33"      | "settle" | "ebhlaunch" | "[EBHLaunch].[dbo].[SETTLING_TABLE]" | "chrome" | "SMRITIDHUNGANA" | "Legendary@1" |
      | "15566"    | "SMRITIDHUNGANA" | "SJAGGI"  | "33"      | "settle" | "ebhlaunch" | "[EBHLaunch].[dbo].[SETTLING_TABLE]" | "MicrosoftEdge" | "SMRITIDHUNGANA" | "Legendary@1" |

## AARLogin should be SJAGGI at Starting##

    #24
  @Regression @AARReassignmentAgentThatSharesVendorCodeWithOtherAgents @FailureScreenShot1
  Scenario Outline: (SET-2298) Add Logic to Check Settling Table Upon AAR Reassignment (Agent that shares a Vendor Code with other Agents)
    Given Run Test for <environment> on Browser <browser> for AAR Agent Management and Enter the url
    And Login to the Agents Portal with username <username> and password <password> for AAR Agent Management
    And Navigate to the Corporate Page on Main Menu and to the Settlements page for AAR Agent Management
    And Navigate to the AAR Agent Management File page
    ...
    And Get the details of Vendor Code as <VendorCode> on AAR Agent Management
    And Check to make sure the Vendor Code <VendorCode> did not change AAR assignment <aarLogin1> upon a CANCEL
    And Update Vendor Code and run the following SQL <environment> and <tableName> <VendorCode> upon CANCEL
    And Click on edit on Action field
    And Reassign AAR Login to your Login from the AAR Login <aarLogin> dropdown
    And Select on SAVE, Click on NO
    And Get the details of Vendor Code as <VendorCode> on AAR Agent Management
    And Check to make sure the Vendor Code <VendorCode> did not change AAR assignment <aarLogin1> upon a SAVE, NO
    And Update Vendor Code and run the following SQL <environment> and <tableName> <VendorCode> upon a SAVE, NO
    And Click on edit on Action field
 ...
 #   And Choose Agent Settling and enter password, Choose Continue, Select OK <Password>
  ...
  #  And Return to the AAR Agent Management form and check filtering to make sure it retains filters when selecting different options <VendorCode> <aarLogin>
    ...
    Examples:
      ...
      | "7517"     | "SMRITIDHUNGANA" | "SJAGGI"  | "50"      | "settle" | "ebhlaunch" | "MicrosoftEdge" | "SMRITIDHUNGANA" | "Legendary@1" | "[EBHLaunch].[dbo].[SETTLING_TABLE]" |
## AARLogin should be SJAGGI at Starting##

    # 25
  @Regression @AARAgentManagementWeeklyCutoffDay/Time @FailureScreenShot1
  Scenario Outline: Validate records selecting CUTOFF DAY/TIME in AAR Agent Management Table
    ...
    Examples:
      ...
      | "WEDNESDAY"     | "06:00 AM"       | "ebhlaunch" | "MicrosoftEdge" | "SMRITIDHUNGANA" | "Legendary@1" | "[EBHLaunch].[dbo].[AGENT_SETTLEMENT_INFO]" |


       # 26
  @Regression @TractorAARLogin @FailureScreenShot1
  Scenario Outline: (SET-2479) Validate, Addition of AGENT_STL_TRAC_AAR for AAR Tractor Assignment
    ...
    Examples:
      ...
      | "SAV"     | "ebhstaging" | "MicrosoftEdge" | "SmritiTest" | "Legendary@1" | "[EBH].[dbo].[AGENT_SETTLEMENT_INFO]" | "[EBH].[dbo].[TRACTOR_SETTLING_TABLE]" |

    # 27
  @Regression @TractorAARLoginNoOneIsCurrentlySettling @FailureScreenShot1
  Scenario Outline: (SET-2479) No-one else is currently settling agent, the agent is VERIFIED or NOT VERIFIED Tractor AAR Login
    Given Run Test for <environment> on Browser <browser> for AAR Agent Management and Enter the url
    And Login to the Agents Portal with username <username> and password <password> for AAR Agent Management
    And Navigate to the Corporate Page on Main Menu and to the Settlements page for AAR Agent Management
    And Navigate to the AAR Agent Management File page
    ...
    Examples:
      ...
      | "TESTTINA1"     | "SMRITITEST"          | "AWG"                             | "ebhstaging" | "MicrosoftEdge" | "SmritiTest" | "Legendary@1" | "[EBH].[dbo].[AGENT_SETTLEMENT_INFO]" | "[EBH].[dbo].[TRACTOR_SETTLING_TABLE]" |


     # 28
  @Regression @TractorAARLoginAnotherAARisSettlingTheAgentVerified @FailureScreenShot1
  Scenario Outline: (SET-2479) Another AAR is settling the agent, the AGENT is VERIFIED, Tractor AAR Login
    Given Run Test for <environment> on Browser <browser> for AAR Agent Management and Enter the url
    And Login to the Agents Portal with username <username> and password <password> for AAR Agent Management
    And Navigate to the Corporate Page on Main Menu and to the Settlements page for AAR Agent Management
    And Navigate to the AAR Agent Management File page
   ...
   # And Upon Adding or Updating Tractor AAR Login, ensure no-one else is settling this agent, the agent is verified, Run SQL <environment> <tableName1> <agentCode> <tableName>
    ...
    Examples:
      ...
      | "TESTTINA1"     | "SMRITITEST"          | "SAV"     | "ebhstaging" | "MicrosoftEdge" | "SmritiTest" | "Legendary@1" | "[EBH].[dbo].[AGENT_SETTLEMENT_INFO]" | "[EBH].[dbo].[TRACTOR_SETTLING_TABLE]" |


     # 29
  @Regression @TractorAARLoginAnotherAARisSettlingTheAgentNOTVerified @FailureScreenShot1
  Scenario Outline: (SET-2479) Another AAR is settling the agent, the AGENT is NOT VERIFIED, Tractor AAR Login
    Given Run Test for <environment> on Browser <browser> for AAR Agent Management and Enter the url
    And Login to the Agents Portal with username <username> and password <password> for AAR Agent Management
    And Navigate to the Corporate Page on Main Menu and to the Settlements page for AAR Agent Management
    And Navigate to the AAR Agent Management File page
  ...
  #  And Upon Adding or Updating Tractor AAR Login, ensure no-one else is settling this agent, the agent is not verified, Run SQL <environment> <tableName1> <agentCode> <tableName>
    ...
    Examples:
      | tractorAARLogin | tractorAARLoginUpdate | agentCode | environment  | browser         | username     | password      | tableName                             | tableName1                             |
      | "ANIL"          | "SMRITITEST"          | "BCO"     | "ebhstaging" | "MicrosoftEdge" | "SmritiTest" | "Legendary@1" | "[EBH].[dbo].[AGENT_SETTLEMENT_INFO]" | "[EBH].[dbo].[TRACTOR_SETTLING_TABLE]" |
